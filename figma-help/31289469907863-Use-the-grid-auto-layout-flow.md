---
기술지원명: Use the grid auto layout flow
카테고리: 디자인
작성자: Figma
승인자: (승인 대기)
출처: Use the grid auto layout flow
출처링크: https://help.figma.com/hc/en-us/articles/31289469907863-Use-the-grid-auto-layout-flow
게시일: (승인 대기)
검토일: (승인 대기)
---
...
Grid is one of three [auto layout flows](https://help.figma.com/hc/en-us/articles/360040451373) that you can apply to frames. A grid consists of “cells” in which you can place layers and components, and organized into rows and columns (tracks). Objects can span multiple cells and have the ability to respond when their parent frame is resized.

While the [vertical and horizontal](https://help.figma.com/hc/en-us/articles/31289464393751) auto layout options allow for items to flow in one direction, grid is great when you need a more structured or complex layout—like bento boxes, dashboards, editorial layouts, and tables.
...
By default, **Number of rows** is set to `auto`. This means the number of rows in the grid will increase or decrease to accommodate the number of cells needed by cell objects. For example, deleting all cell objects from a row will also remove that row.
...
![grid-picker-track-change.gif](https://help.figma.com/hc/article_attachments/31880057373975)

### Resize columns and rows

To resize a track:

1. Select the frame and hover over the top or left side of the frame until the blue pill appears.
2. Hover over a blue pill and a label should appear. This label indicates the current track size or resizing property.
3. Select the label. From there, you can update the track size from the label or the list of tracks in right sidebar.
...
1. Select the frame and hover over the track you want to move.
2. Click and hold the grabber icon next to the label, and drag the track to the desired area. You’ll be able to see where the track will land by the blue line indicator.

**Note**: If any cell objects in the selected track [span](#h_01JT9Q467E13XQNJ2A54N3JV4M) into other tracks, those tracks will also be moved.

![grid-reorder-tracks.gif](https://help.figma.com/hc/article_attachments/40414326822423)
...
You can also decrease the number of rows or columns in a grid from the grid picker in the right sidebar. Cell objects inside the deleted track will be preserved and moved to the nearest cells. Other cell objects in the grid will move around to accommodate

**Note**: If the **Number of rows** on a grid is set to `Auto`, any empty rows—rows without cell objects—will be automatically removed.
...
By default, cell objects in a grid are automatically positioned, in the order of left-to-right starting from the top row, and moving its way down. This means that whenever a cell object is deleted or if a column is added, sibling cell objects will reposition themselves to fill the empty cells. This also means that when you add a cell object to the grid, Figma will try to place it between the cell objects—in layer order—nearest your cursor.

![automatically-or-manually-position-items.gif](https://help.figma.com/hc/article_attachments/40414359336087)

If you want to preserve empty cells in a grid, you can disable the default behavior to position cell objects manually instead. To do this, select the grid frame and click **Toggle automatic positioning** from the right sidebar.

To revert back to the default behavior, you can turn the setting back on by selecting **Toggle automatic positioning** again. Doing so will also set **Number of rows** to **Auto**.
