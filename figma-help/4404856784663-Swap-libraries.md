---
기술지원명: Swap libraries
카테고리: 디자인
작성자: Figma
승인자: (승인 대기)
출처: Swap libraries
출처링크: https://help.figma.com/hc/en-us/articles/4404856784663-Swap-libraries
게시일: (승인 대기)
검토일: (승인 대기)
---
...
Swap libraries to ‌quickly update a file so they use assets (styles, components, and variables) from another library.
...
- Migrate a file to using a different variable collection
...
Swap library lets you replace styles, component instances, and variables in a current file with those of another published library. You can swap any library you have at least `can view` access to.

Swap library only works if the assets you want to replace originate from a different library. In other words, assets that live in the same file as its source cannot be replaced using library swap.

When swapping, Figma looks for assets with matching names between the two participating libraries. This includes components sets and the variants within them. If you've deleted assets or recreated them with different names, Figma won't recognize or match these assets.

### Unmatched assets

If a matching asset isn't found in the selected library, Figma won't swap them and they'll remain connected to the original library.

You can choose to ignore these assets, delete them from the file, or manually swap instances using the **Instance menu**. Learn how to [swap between component instances](https://help.figma.com/hc/en-us/articles/360039150413).

## Before you swap libraries

Before you swap asset libraries, make sure:

- The current file you're working from contains at least one instance, style, or variable from a different library that you wish to swap.
...
Swap components, styles, and variable collections in the current file with matching assets from another library.
...
5. From the **Choose library** dropdown, select a replacement library.
