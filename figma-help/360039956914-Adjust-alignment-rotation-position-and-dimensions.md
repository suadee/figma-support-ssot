---
기술지원명: Adjust alignment, rotation, position, and dimensions
카테고리: 디자인
작성자: Figma
승인자: (승인 대기)
출처: Adjust alignment, rotation, position, and dimensions
출처링크: https://help.figma.com/hc/en-us/articles/360039956914-Adjust-alignment-rotation-position-and-dimensions
게시일: (승인 대기)
검토일: (승인 대기)
---
...
Looking for text alignment? Learn more about [Figma's text alignment options](https://help.figma.com/hc/en-us/articles/360039956634-Explore-text-properties#h_3ba0d87c-0153-4715-a09d-9dcd7077c8e7).
...
- **Snap to geometry:** Used only in [vector edit mode](https://help.figma.com/hc/en-us/articles/360040450213-Vector-networks#Edit_vector_networks). When this setting is on, clicking and dragging a vector point will align it to other vector points.
- **Snap to objects:** Align the centers and outermost points of different objects.
- **Snap to pixel grid:** Align objects to an underlying grid to prevent misaligned pixel errors when exporting elements. The [pixel grid](https://help.figma.com/hc/en-us/articles/360041065034-Adjust-your-zoom-and-view-options#pixel-grid) does not need to be visible for this setting to work.
...
To temporarily disable **Snap to pixel grid** with `Control`, make sure you’re in [vector edit mode](https://help.figma.com/hc/en-us/articles/360040450213-Vector-networks#Edit_vector_networks) and zoomed in to the canvas. To know if you’re zoomed in enough, turn on the pixel grid `⇧ Shift` `'` and zoom in until the grid is visible. Note that the pixel grid does not need to be toggled on to use this shortcut.
...
Caution: Figma uses the rotate CSS transform property to apply rotation to a layer or selection. The CSS transform property uses the opposite directions for determining the angle of rotation, which means you'll see the opposite angle in [Dev Mode](https://help.figma.com/hc/en-us/articles/15023124644247-Guide-to-Dev-Mode).

For example: if you rotate a layer `90°` in the **Design** panel, Figma will translate this to `transform: rotate(-90deg)` in the [Dev Mode](https://help.figma.com/hc/en-us/articles/15023124644247-Guide-to-Dev-Mode).
...
Note: Figma uses the matrix CSS transform property for flip transformations. You can view the CSS values of the transformation in the [Dev Mode](https://help.figma.com/hc/en-us/articles/15023124644247-Guide-to-Dev-Mode).
